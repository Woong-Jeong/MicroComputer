...
void TIMER4_OC_Init(void)
{
	...
	// Time base 설정
	RCC->APB1ENR |= (1 << 2); // 0x04, RCC_APB1ENR TIMER4 Enable // 최대 42MHz
	...
	// Setting the Period
	// PSC, ARR설정하는거 무조건 시험
	TIM4->PSC = 840 - 1;	   // Prescaler=84, 84MHz/84 = 1MHz (1us)
...
void TIM4_IRQHandler(void) // RESET: 0
{
	if ((TIM4->SR & 0x01) != RESET) // Update interrupt flag (10ms)
	{
		...
		if(sec100m >= 10)
		{
			...
			sec1++;
			if(sec1 >= 10)	sec1 = 0;
			LCD_DisplayChar(2,1,sec1+0x30);
		}
		LCD_DisplayChar(2,2,sec100m+0x30);
	}
