...
int main(void)
{
	...
	while(1)
	{
		...
		{
			case SW0_PUSH : 		//SW0
				GPIOG->ODR &= 0x00;	// All LED Off
				GPIOG->ODR |= 0x01;	// LED0 On
				SerialSendChar('0'); // USART6을 통해 PC로 '0' 문자를 전송
			break;

            case SW1_PUSH : 		//SW1
				GPIOG->ODR &= 0x00;	// All LED Off
				GPIOG->ODR |= 0x02;	// LED1 On
				SerialSendChar('1'); // USART6을 통해 PC로 '1' 문자를 전송
			break;

            case SW2_PUSH : 		//SW2
				GPIOG->ODR &= 0x00;	// All LED Off
				GPIOG->ODR |= 0x04;	// LED2 On
				SerialSendChar('2'); // USART6을 통해 PC로 '2' 문자를 전송
			break;

            case SW3_PUSH : 		//SW3
				GPIOG->ODR &= 0x00;	// All LED Off
				GPIOG->ODR |= 0x08;	// LED3 On
				SerialSendString("456"); // USART6을 통해 PC로 “456 ” 문자열을 전송
			break;
        }      
	}
}
...
void USART6_Init(void)
{
	...
	GPIOC->AFR[0]	|= (8<<4*6);	// Connect GPIOC pin6 to AF8(USART6)
    
	// USART6 : RX(PC7)
	GPIOC->MODER	|= (2<<2*7);	// GPIOC PIN6 Output Alternate function mode					
	GPIOC->OSPEEDR	|= (3<<2*7);	// GPIOC PIN6 Output speed (100MHz Very High speed)
	...
	//USART6->CR1	|= (1<<10);			// USE USART_Parity
	USART6->CR1	&= ~(1<<10);			// NO USE USART_Parity
	//USART6->CR1	&= ~(1<<9);	        // EVEN_Parity config
