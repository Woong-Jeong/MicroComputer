...
int main(void)
{
	...
	while(1)
	{
		...
		case SW3_PUSH : 	//SW3
			...
			SerialSendString("AT+BTNAME=WED21");
			SerialSendChar(0x0D); // '\r' carriage return
			LCD_DisplayText(4,0,"                                    "); 
			LCD_DisplayText(4,0,"AT+BTNAME=WED21"); 
			LCD_DisplayText(6,0,"                               ");
			LCD_DisplayText(7,0,"                               ");
			Px= 0;  Py=0;
 		break;
...
{       
    ...
    // #define  USART_SR_RXNE ((uint16_t)0x0020)    //  Read Data Register Not Empty(FULL)     
	{
		...
		if ((ch != 0x0D)&&(ch != 0x0A)){	//0x0D, 0x0A문자 제거하는 구문
...
{ 
	...
	tmpreg = (integerdivider / 100) << 4; 		// BRR Reg에 정수부분 넣기
  
	// Find the fractional part 
	fractionaldivider = integerdivider - (100 * (tmpreg >> 4));		// BRR Reg에 소수부분 넣기
