...
{     
	...
	ADC1->CR1 |=  (1<<5);		// EOCIE=1: Interrupt enable for EOC	//인터럽트 시작

	ADC1->CR2 &= ~(1<<1);		// CONT=0: ADC_Continuous ConvMode Disable	// 연속으로 안하고 싱글모드로
	ADC1->CR2 |=  (2<<28);		// ****EXTEN[1:0]: ADC_ExternalTrigConvEdge_Enable(Falling Edge)	//에지설정

	//*****************//
	ADC1->CR2 |= (0x0F<<24);	// ****EXTSEL[3:0]: ADC_ExternalTrig (EXTI11) //트리거설정
